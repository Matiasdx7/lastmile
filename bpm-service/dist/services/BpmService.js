...
class BpmService {
    constructor() {
        ...
        this.dispatchRepository = new DispatchRepository_1.DispatchRepository();
        this.loadRepository = new LoadRepository_1.LoadRepository();
        this.routeRepository = new RouteRepository_1.RouteRepository();
        this.vehicleRepository = new VehicleRepository_1.VehicleRepository();
    }
    ...
    async generateOrderProcessDiagram(orderId) {
        try {
            ...
            throw new Error(`Failed to generate process diagram for order ${orderId}`);
        }
    }
    async getProcessNodeDetails(nodeId) {
        try {
            ...
            throw new Error(`Failed to get details for process node ${nodeId}`);
        }
    }
    async getProcessStageCounts() {
        try {
            const counts = {};
            const orderStatusCounts = await this.orderRepository.getOrderCountsByStatus();
    ...
    async getRecentOrdersInStage(stageId, limit) {
        try {
            const statuses = BpmModel_1.processStageMap[stageId];
            if (!statuses || statuses.length === 0) {
                return [];
            }
            const orders = await this.orderRepository.findByStatuses(statuses, limit);
